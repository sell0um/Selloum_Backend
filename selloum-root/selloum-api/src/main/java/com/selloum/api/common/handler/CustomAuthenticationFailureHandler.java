...
public class CustomAuthenticationFailureHandler implements AuthenticationFailureHandler{
	...
	public void onAuthenticationFailure(HttpServletRequest request, HttpServletResponse response,
        ...
        if (exception instanceof BadCredentialsException) {
            code = ErrorCode.PASSWORD_MISMATCH; // 비밀번호 불일치
        } else if (exception instanceof UsernameNotFoundException) {
            code = ErrorCode.USER_NOT_FOUND; // 존재하지 않는 사용자
        } else if (exception instanceof LockedException) {
            code = ErrorCode.INACTIVE_USER; // 계정 잠김
        } else if (exception instanceof DisabledException) {
            code = ErrorCode.INACTIVE_USER; // 비활성화된 계정
        } else if (exception instanceof AccountExpiredException) {
            code = ErrorCode.INACTIVE_USER; // 계정 만료
        } else if (exception instanceof CredentialsExpiredException) {
            code = ErrorCode.PASSWORD_MISMATCH; // 비밀번호 만료
        }

        // 응답 생성
        BaseResponse<Object> responseBody = BaseResponse.of(code);
